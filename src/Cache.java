...
public class Cache {
    ...
    public Cache( int number_rows,  int set_associativity, int cache_blocksize, Memory mem, String replacement_policy, String write_policy) {
  ...
  public Cache_Block replacement_policy(Cache_Set my_set) {
	  Cache_Block block = null;
	  if( getReplacement_policy().equals("LRU")|| getReplacement_policy().equals("lru") ) {
		  block = my_set.lru();
      }
      else if( getReplacement_policy().equals("REF")|| getReplacement_policy().equals("ref") ) {
    	  block =  my_set.ref();
      }
      else {
    	  //
